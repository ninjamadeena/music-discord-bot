...
client.on("messageCreate", async (msg) => {
  try {
    ...
    if (cmd === "skip") {
      ...
      state.isForceTransition = true; // เพิ่มเติม Guard สำหรับข้ามเพลง
...
function createGuildState(guild) {
  ...
  const state = {
    ...
    isForceTransition: false, // เพิ่ม: เช็คว่าสั่ง skip/stop เองหรือเปล่า (ถ้าใช่ให้ข้าม error ไป)
    controlMsg: null,
  };
...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  try {
    ...
    // ส่วนที่อัปเกรด: รอให้เสียงเล่นจริงๆ ก่อนส่ง Now Playing
