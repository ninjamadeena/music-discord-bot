...
const config = {
  ...
  opusComplexity: Math.max(0, Math.min(10, Number(process.env.OPUS_COMPLEXITY) ?? 5)),
...
function logPretty(type, msg, extra = {}) {
  ...
  const plain = (`[${nowStr()}] [${cfg.label}] ${msg} | ping=${ws}ms` +
  ...
  if (!isDebug || DEBUG_FFMPEG) console.log(line);
}
...
function checkFfmpegAvailability(){
  ...
  try {
    ...
    if (!res.error && res.status === 0) { FFMPEG_AVAILABLE = true; return; }
  } catch {}
...
function buildHelpEmbedPrefix(){
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "🎶  เล่นเพลง & จัดการคิว", value: `\`${p}play <ชื่อ/URL>\` — เล่นหรือเพิ่มเพลงเข้าคิว\n\`${p}playlist <URL/คำค้น> --limit N\` — โหลดเพลงเป็นชุด\n\`${p}queue\` — ดูรายการคิวทั้งหมด\n\`${p}np\` — เพลงที่กำลังเล่นอยู่\n\`${p}remove <เลข>\` — ลบเพลงออกจากคิว\n\`${p}shuffle\` — สุ่มลำดับคิว`, inline: false },
      { name: "⏯️  ควบคุมการเล่น", value: `\`${p}skip\` — ข้ามเพลงปัจจุบัน\n\`${p}pause\` — หยุดชั่วคราว\n\`${p}resume\` — เล่นต่อ\n\`${p}stop\` — หยุดและล้างคิวทั้งหมด`, inline: true },
      { name: "🔊  เสียง & การวน", value: `\`${p}volume <0-10000>\` — ปรับระดับเสียง\n\`${p}loop off\` — ปิดการวน\n\`${p}loop track\` — วนเพลงเดิม\n\`${p}loop queue\` — วนทั้งคิว\n\`${p}autoplay\` — เปิด/ปิด สุ่มเพลงอัตโนมัติ`, inline: true },
      { name: "⚙️  ระบบ", value: `\`${p}ping\` — ตรวจสอบ latency\n\`${p}botupdate\` — อัปเดต yt-dlp\n\`${p}help\` — แสดงคู่มือนี้`, inline: false },
      { name: "╚══════════════════════════╝", value: "** **", inline: false }
    )
...
function buildHelpEmbedSlash(){
  return new EmbedBuilder()
    ...
    .addFields(
      { name: "╔══════════════════════════╗", value: "** **", inline: false },
      { name: "🎶  เล่นเพลง & จัดการคิว", value: "`/play query:<ชื่อ/URL>` — เล่นหรือเพิ่มเพลงเข้าคิว\n`/playlist query:<URL/คำค้น> limit:<N>` — โหลดเพลงเป็นชุด\n`/queue` — ดูรายการคิวทั้งหมด\n`/np` — เพลงที่กำลังเล่นอยู่\n`/remove index:<เลข>` — ลบเพลงออกจากคิว\n`/shuffle` — สุ่มลำดับคิว", inline: false },
      { name: "⏯️  ควบคุมการเล่น", value: "`/skip` — ข้ามเพลงปัจจุบัน\n`/pause` — หยุดชั่วคราว\n`/resume` — เล่นต่อ\n`/stop` — หยุดและล้างคิวทั้งหมด", inline: true },
      { name: "🔊  เสียง & การวน", value: "`/volume value:<0-10000>` — ปรับระดับเสียง\n`/loop mode:off` — ปิดการวน\n`/loop mode:track` — วนเพลงเดิม\n`/loop mode:queue` — วนทั้งคิว\n`/autoplay` — เปิด/ปิด สุ่มเพลงอัตโนมัติ", inline: true },
      { name: "⚙️  ระบบ", value: "`/ping` — ตรวจสอบ latency\n`/botupdate` — อัปเดต yt-dlp\n`/help` — แสดงคู่มือนี้", inline: false },
      { name: "╚══════════════════════════╝", value: "** **", inline: false }
    )
...
const COLORS = { primary: 0x5865F2, success: 0x57F287, warning: 0xFEE75C, error: 0xED4245, info: 0x5DADE2, music: 0xE91E63, queue: 0x9B59B6 };
function makeEmbed(color = COLORS.primary) { return new EmbedBuilder().setColor(color).setTimestamp(); }
function successEmbed(title, description) { return makeEmbed(COLORS.success).setDescription(`### ${title}\n${description ? `${description}` : ""}`); }
function errorEmbed(description) { return makeEmbed(COLORS.error).setDescription(`### ❌  เกิดข้อผิดพลาด\n${description}`); }
function infoEmbed(title, description) { return makeEmbed(COLORS.info).setDescription(`### ${title}\n${description ? `${description}` : ""}`); }
function musicEmbed(title, description) { return makeEmbed(COLORS.music).setDescription(`### ${title}\n${description ? `${description}` : ""}`); }
function loopLabel(mode) { if (mode === "track") return "🔂 วนเพลงเดิม"; if (mode === "queue") return "🔁 วนทั้งคิว"; return "➡️ ปิด"; }

function buildMusicControlRow(isPaused = false) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("mc_pause_resume").setLabel(isPaused ? "▶️  เล่นต่อ" : "⏸️  หยุดชั่วคราว").setStyle(isPaused ? ButtonStyle.Success : ButtonStyle.Primary),
    new ButtonBuilder().setCustomId("mc_skip").setLabel("⏭️  ข้ามเพลง").setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId("mc_stop").setLabel("⏹️  ปิดระบบ").setStyle(ButtonStyle.Danger),
  );
}
...
function buildNowPlayingEmbed(title, requestedBy, volumePct, ping, nextTitle, isPaused = false) {
  ...
  return makeEmbed(isPaused ? COLORS.warning : COLORS.music).setDescription(`### 🎶  Now Playing\n${statusLine}`).addFields(
...
async function updateControlMsgIfNeeded(state) {
  ...
  try {
    ...
    await state.controlMsg.edit({
      embeds: [buildNowPlayingEmbed(state.current.title, state.current.requestedBy, state.volumePct, wsPing(), state.queue[0]?.title || null, isPaused)],
...
async function replyAck(msg, text){ try { return await msg.channel.send({ content: text }); } catch { return null; } }

async function getTitle(input){
  try {
    if (config.cacheEnabled) {
      const ytMatch = input.match(/(?:youtu\.be\/|youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=))([^"&?\/\s]{11})/i);
      if (ytMatch && cacheDB[ytMatch[1]]) return cacheDB[ytMatch[1]].title; 
      
      if (!isUrl(input)) {
        const lowerQ = input.toLowerCase();
        for (const data of Object.values(cacheDB)) {
          if (data.title.toLowerCase().includes(lowerQ)) return data.title;
        }
      }
    }

    if (isSpotifyUrl(input)) { const t = await spotifyTitle(input); if (t) return t; }
    
    const searchInput = isUrl(input) ? input : `ytsearch1:${input}`;
    const args = ["--dump-single-json", "--flat-playlist", "--no-check-certificates", "--js-runtimes", "node", searchInput];
    if (config.ytdlpForceIpv4) args.unshift("--force-ipv4");
    if (config.cookieFile) args.push("--cookies", config.cookieFile);

    const info = await ytdlp(args).catch(() => null);
    
    if (info && info._type === "playlist" && info.entries && info.entries.length > 0) return info.entries[0].title;
...
client.on("messageCreate", async (msg) => {
  try {
    ...
    const cmd = ({ p: "play", q: "queue", now: "np", next: "skip", s: "skip", st: "stop", vol: "volume", upd: "botupdate", h: "help", help: "help", ap: "autoplay", auto: "autoplay" })[cmdRaw] || cmdRaw;

    logPretty("PREFIX", `${BOT_PREFIX}${cmd}`, { user: msg.author.tag, guild: msg.guild.name });
    const allowed = new Set(["help","play","playlist","skip","stop","pause","resume","queue","np","remove","shuffle","loop","volume","ping","botupdate","autoplay"]);
    if (!allowed.has(cmd)) {
      return msg.reply({ embeds: [errorEmbed(`ไม่รู้จักคำสั่ง \`${BOT_PREFIX}${cmdRaw}\``).addFields({ name: "💡 คำสั่งที่รองรับ", value: Array.from(allowed).map(c=>`\`${BOT_PREFIX}${c}\``).join(" ") })]});
    }

    if (cmd === "help") return msg.reply({ embeds: [buildHelpEmbedPrefix()] });

    let q = null, limit = null, index = null, mode = null, value = null;
    if (cmd === "play") { q = parts.join(" ").trim(); if (!q) return msg.reply({ embeds: [errorEmbed(`กรุณาระบุชื่อเพลงหรือลิงก์\n**ตัวอย่าง:** \`${BOT_PREFIX}play lofi hip hop\``)] }); }
    else if (cmd === "playlist") { const parsed = parseLimitFromArgs(parts); limit = parsed.limit; q = parsed.tokens.join(" ").trim(); if (!q) return msg.reply({ embeds: [errorEmbed(`กรุณาระบุลิงก์ playlist หรือคำค้น\n**ตัวอย่าง:** \`${BOT_PREFIX}playlist lofi playlist --limit 20\``)] }); }
    else if (cmd === "remove") { index = parseInt(parts[0], 10); if (Number.isNaN(index) || index < 1) return msg.reply({ embeds: [errorEmbed(`กรุณาระบุหมายเลขลำดับ\n**ตัวอย่าง:** \`${BOT_PREFIX}remove 3\``)] }); }
    else if (cmd === "loop") { mode = (parts[0] || "").toLowerCase(); if (!mode) return msg.reply({ embeds: [errorEmbed(`กรุณาระบุโหมด: \`off\` · \`track\` · \`queue\`\n**ตัวอย่าง:** \`${BOT_PREFIX}loop track\``)] }); }
    else if (cmd === "volume") { value = parseInt(parts[0], 10); if (Number.isNaN(value)) return msg.reply({ embeds: [errorEmbed(`กรุณาระบุตัวเลขความดัง (0–10000)\n**ตัวอย่าง:** \`${BOT_PREFIX}volume 80\``)] }); }

    const me = msg.guild.members.me;
    const userVC = msg.member?.voice?.channelId;
    const botVC = me?.voice?.channelId;
    const sameVC = userVC && (!botVC || botVC === userVC);
    const needsSameVC = !["help","ping", "botupdate", "np", "queue", "autoplay"].includes(cmd);
    
    if (needsSameVC && !sameVC) return msg.reply({ embeds: [errorEmbed("กรุณาเข้าห้องเสียงเดียวกับบอทก่อนนะ 🎙️")] });
    ...
    if (cmd === "ping") return msg.reply({ embeds: [makeEmbed(COLORS.info).setDescription("### 🏓  Pong!").addFields({ name: "🌐 WebSocket", value: `\`${Math.round(msg.client.ws.ping)} ms\``, inline: true })]});
    if (cmd === "botupdate") {
      const m = await msg.reply({ embeds: [infoEmbed("🔄  กำลังอัปเดต yt-dlp…", "โปรดรอสักครู่")] });
      await runYtDlpUpdate((t) => m.edit({ embeds: [t.startsWith("✅") ? successEmbed("✅  อัปเดตสำเร็จ", "yt-dlp อัปเดตเรียบร้อยแล้ว") : errorEmbed(t)], content: "" }));
      return;
    }

    if (cmd === "autoplay") {
      state.autoPlay = !state.autoPlay;
      return msg.reply({ embeds: [successEmbed("📻  Auto-Play", `สถานะ: **${state.autoPlay ? "เปิดการใช้งาน" : "ปิดการใช้งาน"}**\n*(สุ่มเพลงจาก Cache เล่นอัตโนมัติเมื่อคิวหมด)*`)]});
    }
    ...
    if (cmd === "play") {
      ...
      await msg.reply({ embeds: [makeEmbed(COLORS.success).setDescription(`### ➕  เพิ่มเพลงเข้าคิวแล้ว`).addFields({ name: "🎵 เพลง", value: `**${title}**`, inline: false }, { name: "📋 ลำดับในคิว", value: `\`#${state.queue.length}\``, inline: true }, { name: "👤 ขอโดย", value: `${msg.author}`, inline: true })]});
      if (!state.current) playNext(msg.guild, msg.channelId, state); else updateControlMsgIfNeeded(state);
      return;
    }
    ...
    if (cmd === "playlist") {
      ...
      for (const { title, url } of items) state.queue.push({ title, source: url, requestedBy: msg.author.tag, guild: msg.guild, voiceChannelId: userVC, textChannelId: msg.channelId });
      const preview = items.slice(0, 5).map((x, i) => `\`${i + 1}.\` ${x.title}`).join("\n");
      const more = items.length > 5 ? `\n*… และอีก ${items.length - 5} เพลง*` : "";
      await msg.reply({ embeds: [makeEmbed(COLORS.queue).setDescription(`### 📚  โหลด Playlist สำเร็จ`).addFields({ name: "🎶 เพลงทั้งหมด", value: `**${items.length} เพลง**`, inline: true }, { name: "👤 ขอโดย", value: `${msg.author}`, inline: true }, { name: "📋 รายการแรก", value: `${preview}${more}`, inline: false })]});
      if (!state.current) playNext(msg.guild, msg.channelId, state); else updateControlMsgIfNeeded(state);
      return;
    }
    ...
    if (cmd === "skip") {
      ...
      if (state.queue.length === 0 && state.loopMode !== "queue" && !state.autoPlay) {
        state.isForceTransition = true; state.current = null; state.loopMode = "off"; state.skipRequested = false;
        if (state.controlMsg) {
          try { await state.controlMsg.edit({ components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("mc_pause_resume").setLabel("⏸️  หยุดชั่วคราว").setStyle(ButtonStyle.Primary).setDisabled(true), new ButtonBuilder().setCustomId("mc_skip").setLabel("⏭️  ข้ามเพลง").setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId("mc_stop").setLabel("⏹️  ปิดระบบ").setStyle(ButtonStyle.Danger).setDisabled(true))]}); } catch {}
          state.controlMsg = null;
        }
        cleanupCurrentPipeline(state); state.player.stop(true);
        const vc = getVoiceConnection(msg.guild.id); if (vc) vc.destroy();
        return msg.reply({ embeds: [successEmbed("⏭️  ข้ามเพลงสุดท้ายแล้ว!", "คิวว่างเปล่า ขอตัวไปนอนพักก่อนนะ 👋")] });
      }
      
      state.isForceTransition = true; state.skipRequested = true; state.skipGeneration = (state.skipGeneration || 0) + 1;
      if (state.controlMsg) {
        try { await state.controlMsg.edit({ components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("mc_pause_resume").setLabel("⏸️  หยุดชั่วคราว").setStyle(ButtonStyle.Primary).setDisabled(true), new ButtonBuilder().setCustomId("mc_skip").setLabel("⏭️  ข้ามเพลง").setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId("mc_stop").setLabel("⏹️  ปิดระบบ").setStyle(ButtonStyle.Danger).setDisabled(true))]}); } catch {}
        state.controlMsg = null;
      }
      cleanupCurrentPipeline(state); state.player.stop(true);
      const nextLabel = state.queue.length ? `ถัดไป: **${state.queue[0]?.title || "—"}**` : (state.autoPlay ? "📻 สุ่มเพลงถัดไปจากสถานี..." : "คิวหมดแล้ว");
      return msg.reply({ embeds: [successEmbed("⏭️  ข้ามเพลงแล้ว", nextLabel)] });
    }

    if (cmd === "stop") {
      state.isForceTransition = true; state.queue = []; state.current = null; state.loopMode = "off"; state.skipRequested = false;
      if (state.controlMsg) {
        try { await state.controlMsg.edit({ components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("mc_pause_resume").setLabel("⏸️  หยุดชั่วคราว").setStyle(ButtonStyle.Primary).setDisabled(true), new ButtonBuilder().setCustomId("mc_skip").setLabel("⏭️  ข้ามเพลง").setStyle(ButtonStyle.Secondary).setDisabled(true), new ButtonBuilder().setCustomId("mc_stop").setLabel("⏹️  ปิดระบบ").setStyle(ButtonStyle.Danger).setDisabled(true))]}); } catch {}
        state.controlMsg = null;
      }
    ...
    if (cmd === "pause") { state.player.pause(); updateControlMsgIfNeeded(state); return msg.reply({ embeds: [infoEmbed("⏸️  หยุดชั่วคราว", "พิมพ์ `n!resume` เพื่อเล่นต่อ")] }); }
    if (cmd === "resume") { state.player.unpause(); updateControlMsgIfNeeded(state); return msg.reply({ embeds: [successEmbed("▶️  เล่นต่อแล้ว", `กำลังเล่น: **${state.current?.title || "—"}**`)] }); }

    if (cmd === "np") {
      if (!state.current) return msg.reply({ embeds: [infoEmbed("🎵  ไม่มีเพลงกำลังเล่น", "ใช้ `n!play <ชื่อเพลง>` เพื่อเริ่มเล่น")] });
      return msg.reply({ embeds: [makeEmbed(COLORS.music).setDescription(`### 🎶  Now Playing`).addFields({ name: "🎵 เพลง", value: `**${state.current.title}**`, inline: false }, { name: "👤 ขอโดย", value: state.current.requestedBy, inline: true }, { name: "🔊 ระดับเสียง", value: `${state.volumePct}%`, inline: true }, { name: "🔁 Loop", value: loopLabel(state.loopMode), inline: true }, { name: "📋 คิวที่เหลือ", value: `${state.queue.length} เพลง`, inline: true })]});
    }
    ...
    if (cmd === "queue") {
      ...
      return msg.reply({ embeds: [makeEmbed(COLORS.queue).setDescription(`### 📋  คิวเพลง`).addFields({ name: `รายการ (${Math.min(state.queue.length, 10)}/${state.queue.length})`, value: lines + more, inline: false }, { name: "🔁 Loop", value: loopLabel(state.loopMode), inline: true }, { name: "🎵 กำลังเล่น", value: state.current ? `**${state.current.title}**` : "—", inline: true })]});
    }

    if (cmd === "volume") {
      setVolumePct(state, value); updateControlMsgIfNeeded(state);
    ...
    if (cmd === "shuffle") { shuffleArray(state.queue); updateControlMsgIfNeeded(state); return msg.reply({ embeds: [successEmbed("🔀  สุ่มคิวแล้ว", `สลับลำดับ **${state.queue.length} เพลง** เรียบร้อย`)] }); }
    if (cmd === "remove") {
      if (index > state.queue.length) return msg.reply({ embeds: [errorEmbed(`หมายเลขลำดับเกินจำนวนในคิว (มีอยู่ ${state.queue.length} เพลง)`)] });
      const [rm] = state.queue.splice(index - 1, 1); updateControlMsgIfNeeded(state);
    ...
    if (cmd === "loop") {
      ...
      state.loopMode = mode; return msg.reply({ embeds: [successEmbed("🔁  ตั้งค่า Loop แล้ว", `โหมดปัจจุบัน: **${loopLabel(mode)}**`)] });
    }
...
const commands = [
  new SlashCommandBuilder().setName("play").setDescription("เล่นเพลงจาก YouTube (ชื่อเพลงหรือ URL)").addStringOption(o => o.setName("query").setDescription("ชื่อเพลง/URL").setRequired(true)),
  ...
  new SlashCommandBuilder().setName("autoplay").setDescription("เปิด/ปิด ระบบสุ่มเพลงจากเครื่อง (Cache) เล่นอัตโนมัติเมื่อคิวว่าง"),
  ...
  new SlashCommandBuilder().setName("volume").setDescription("ปรับความดัง (0-10000)").addIntegerOption(o => o.setName("value").setDescription("เปอร์เซ็นต์ (0-10000)").setRequired(true).setMinValue(0).setMaxValue(10000)),
  new SlashCommandBuilder().setName("playlist").setDescription("เพิ่มเพลงเป็นชุดจาก YouTube (playlist หรือผลค้นหา)").addStringOption(o => o.setName("query").setDescription("ลิงก์ playlist หรือคำค้น").setRequired(true)).addIntegerOption(o => o.setName("limit").setDescription("จำนวนสูงสุด (ถ้าไม่ใส่ = ทั้ง playlist) (1-5000)").setMinValue(1).setMaxValue(5000)),
  new SlashCommandBuilder().setName("remove").setDescription("ลบเพลงจากคิวตามลำดับ").addIntegerOption(o => o.setName("index").setDescription("ลำดับเพลงตาม /queue").setRequired(true).setMinValue(1)),
  new SlashCommandBuilder().setName("shuffle").setDescription("สลับลำดับคิวแบบสุ่ม"),
  new SlashCommandBuilder().setName("loop").setDescription("ตั้งค่าการวนเพลง/คิว").addStringOption(o => o.setName("mode").setDescription("รูปแบบการวน").setRequired(true).addChoices({ name: "ปิด", value: "off" }, { name: "วนเพลงปัจจุบัน", value: "track" }, { name: "วนทั้งคิว", value: "queue" })),
...
function createGuildState(guild) {
  ...
  const state = { queue: [], current: null, player, currentPipe: null, restartGuard: { tried: false }, currentResource: null, volumePct: config.defaultVolume, loopMode: config.defaultLoop, autoPlay: false, skipRequested: false, skipGeneration: 0, isHandlingTransition: false, isForceTransition: false, controlMsg: null };
  player.on(AudioPlayerStatus.Idle, () => { if (!state.isHandlingTransition) handlePlayerIdle(guild, state).catch((e) => logPretty("ERROR", `Idle handler error: ${e?.message || e}`)); });
  player.on("error", (e) => { handlePlayerError(e, guild, state).catch((err) => logPretty("ERROR", `Player error handler failed: ${err?.message || err}`)); });
  return state;
}

function getGuildState(guild) {
  let state = guildStates.get(guild.id);
  if (!state) { state = createGuildState(guild); guildStates.set(guild.id, state); }
  return state;
}

async function sendToTextChannel(guild, textChannelId, content){
  try { const ch = guild.channels.cache.get(textChannelId); if (ch && ch.isTextBased?.()) return ch.send(content); } catch {}
}

function ensureVC(guild, channelId, state){
  let conn = getVoiceConnection(guild.id);
  if (!conn) conn = joinVoiceChannel({ channelId, guildId: guild.id, adapterCreator: guild.voiceAdapterCreator, selfDeaf: true });
  if (state) conn.subscribe(state.player);
  return conn;
}
...
function cleanupCurrentPipeline(state){
  ...
  } catch (e) { swallowPipeError(e); } finally { state.currentPipe = null; }
}
...
async function fetchPlaylistEntries(input, limit = null) {
  ...
  let max;
  if (limit === null || limit === undefined) max = isInputUrl ? Infinity : 25;
  else max = Math.min(Math.max(Number(limit) || 25, 1), hardCap);
  ...
  } catch (err) { logPretty("ERROR", "fetchPlaylistEntries fail: " + (err?.message || err)); }
...
async function handlePlayerIdle(guild, state) {
  ...
  if (state.loopMode === "track" && !manualSkip) { state.restartGuard.tried = false; await playSame(guild, savedChannelId, finished, state); return; }
  if (state.loopMode === "queue") { state.queue.push({ ...finished }); }
...
async function handlePlayerError(error, guild, state) {
  ...
  if (!state.current || state.isHandlingTransition) return; 
  if (state.isForceTransition) { logPretty("LOG", `Ignored error because of manual skip/stop: ${error?.message || error}`); return; }

  const currentItem = state.current;
  const savedChannelId = currentItem.textChannelId;

  if (state._activePlayGen !== state.playGeneration) { logPretty("LOG", `Player error from previous song (ignored, gen mismatch): ${error?.message || error}`); return; }
  ...
  if (!state.restartGuard.tried) {
    ...
    await sendToTextChannel(guild, savedChannelId, { embeds: [makeEmbed(COLORS.warning).setDescription("### 🔁  สัญญาณหลุด\nกำลังลองเชื่อมต่อใหม่…")]});
...
// 🟢 อัปเกรด playNext ให้ทำงานสอดคล้องกับ Auto-Play (หยิบไฟล์ Cache มาสุ่มเล่น) 🟢
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  if (!state.queue.length) {
    ...
    // ดึงรายชื่อเพลงทั้งหมดใน cmusic.json ที่มีไฟล์จริงอยู่ในฮาร์ดดิสก์
    const validCacheKeys = Object.keys(cacheDB).filter(k => fs.existsSync(cacheDB[k].file_path));

    if (state.autoPlay && validCacheKeys.length > 0 && vc) {
      // 📻 โหมดสถานีวิทยุทำงาน! สุ่มเพลงจาก Cache มายัดเข้าคิว
      const randomId = validCacheKeys[Math.floor(Math.random() * validCacheKeys.length)];
      const songData = cacheDB[randomId];
      
      state.queue.push({
        title: songData.title,
        source: `https://www.youtube.com/watch?v=${randomId}`,
        requestedBy: "Auto-Play 📻",
        guild: guild,
        voiceChannelId: vc.joinConfig.channelId,
        textChannelId: textChannelId
      });
      logPretty("INFO", `[AUTO-PLAY] สุ่มได้เพลง: ${songData.title}`);
      
      // ปล่อยให้โค้ดไหลลงไปข้างล่างเพื่อดึงเพลงที่เพิ่งเข้าคิวมาเล่น
    } else {
      // โหมดปกติ คิวหมดก็เตะบอทออก
      state.current = null; state.isForceTransition = false; 
      if (vc) vc.destroy();
      ...
      await sendToTextChannel(guild, textChannelId, { embeds: [makeEmbed(COLORS.info).setDescription("### 📭  คิวหมดแล้ว\nเพิ่มเพลงใหม่ด้วย `/play` หรือ `n!play` ได้เลย!")]});
      return;
    }
  }
  ...
  try {
    ...
    logPretty("NOWPLAY", `${next.title}`, { user: next.requestedBy, tail: `up_next: ${state.queue[0]?.title || (state.autoPlay ? "Auto-Play 📻" : "—")}` });

    await new Promise((resolve) => {
      if (state.player.state.status === AudioPlayerStatus.Playing) { state.isForceTransition = false; return resolve(); }
      const onStateChange = (oldState, newState) => {
        if (newState.status === AudioPlayerStatus.Playing) { state.player.removeListener("stateChange", onStateChange); clearTimeout(timeout); state.isForceTransition = false; resolve(); }
      };
      state.player.on("stateChange", onStateChange);
      const timeout = setTimeout(() => { state.player.removeListener("stateChange", onStateChange); state.isForceTransition = false; resolve(); }, 15000);
    });
    ...
    await sendToTextChannel(guild, next.textChannelId, {
      embeds: [buildNowPlayingEmbed(next.title, next.requestedBy, state.volumePct, ws, state.queue[0]?.title || (state.autoPlay ? "สุ่มเพลงถัดไปจากสถานี 📻" : null), false)],
    ...
    await sendToTextChannel(guild, next.textChannelId, { embeds: [makeEmbed(COLORS.warning).setDescription(`### ⚠️  ข้ามเพลงนี้แล้ว\nเกิดปัญหากับ **${next?.title ?? "ไม่ทราบชื่อ"}**`)]});
    state.current = null;
...
async function playSame(guild, textChannelId, item, state = getGuildState(guild)) {
  try {
    state.current = item; cleanupCurrentPipeline(state); await startPlayback(guild, item, state);
    logPretty("WARN", `RESTARTED  ${item.title}`);
  } catch (err) {
    logPretty("ERROR", "playSame error: " + (err?.message || err));
    state.current = null; await playNext(guild, textChannelId, state);
  }
}

function applyVolume(state) {
  try { const pct = Number.isFinite(state.volumePct) ? state.volumePct : 100; state.currentResource?.volume?.setVolumeLogarithmic(Math.max(pct, 0) / 100); } catch {}
}
...
async function startPlayback(guild, item, state) {
  ...
  if (config.cacheEnabled) {
    const ytMatch = source.match(/(?:youtu\.be\/|youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=))([^"&?\/\s]{11})/i);
    
    if (ytMatch) {
      const vId = ytMatch[1];
      if (cacheDB[vId] && fs.existsSync(cacheDB[vId].file_path)) {
        fastCacheHit = { id: vId, ...cacheDB[vId] };
      }
    } else if (!isUrl(source)) {
      const lowerQ = source.toLowerCase();
      for (const [id, data] of Object.entries(cacheDB)) {
        if (data.title.toLowerCase().includes(lowerQ)) {
          if (fs.existsSync(data.file_path)) {
            fastCacheHit = { id, ...data };
            break;
          }
        }
      }
  ...
  if (fastCacheHit) {
    logPretty("LOG", `[FAST CACHE HIT] ⚡ ${fastCacheHit.title}`);
    videoId = fastCacheHit.id;
    title = fastCacheHit.title;
    filePath = fastCacheHit.file_path;
    targetUrl = `https://www.youtube.com/watch?v=${videoId}`;
    playFromCache = true;
    
    cacheDB[videoId].last_played = Date.now();
    saveCacheDB();
  } else {
    ...
    const ytdlpArgsInfo = ["--js-runtimes", "node", "--dump-single-json", source];
    if (config.ytdlpForceIpv4) ytdlpArgsInfo.unshift("--force-ipv4");
    
    let info = await ytdlp(ytdlpArgsInfo).catch(() => null);
    
    if (info && info._type === "playlist" && info.entries && info.entries.length > 0) {
      info = info.entries[0];
    }
    
    if (!info || !info.id) {
      logPretty("WARN", "ดึงข้อมูลล้มเหลว สลับไปเล่นผ่าน Pipe (Fallback)");
      const pipeObj = spawnUniversalPipe(source);
      state.currentPipe = pipeObj;
      const resource = createAudioResource(pipeObj.stream, { inputType: StreamType.OggOpus, inlineVolume: true });
    ...
    targetUrl = `https://www.youtube.com/watch?v=${videoId}`; 
    filePath = path.join(CACHE_DIR, `${videoId}.ogg`);

    if (config.cacheEnabled && cacheDB[videoId] && fs.existsSync(cacheDB[videoId].file_path)) {
       ...
       const dlArgs = [
         ...
         "--js-runtimes", "node",
         "--extractor-args", "youtube:player-client=web,web_creator;player_skip=webpage",
         "--force-ipv4",
         "-f", "bestaudio/best",
         "-x", "--audio-format", "opus",
  ...
  if (playFromCache) {
      ...
      const pipeObj = spawnUniversalPipe(targetUrl); 
      state.currentPipe = pipeObj;
...
client.on("interactionCreate", async (itx) => {
  if (itx.isButton()) {
    ...
    if (!itx.guild) return await itx.reply({ content: "ใช้ได้ในเซิร์ฟเวอร์เท่านั้น", flags: [64] });
    ...
    if (!userVC || (botVC && botVC !== userVC)) {
      return itx.reply({ embeds: [errorEmbed("กรุณาเข้าห้องเสียงเดียวกับบอทก่อนนะ 🎙️")], flags: [64] });
    }
        ...
        try {
          await state.controlMsg.edit({
            embeds: [buildNowPlayingEmbed(
              ...
              state.volumePct, wsPing(), state.queue[0]?.title || (state.autoPlay ? "สุ่มเพลงถัดไปจากสถานี 📻" : null), nowPaused
            )],
    ...
    if (id === "mc_skip") {
      if (!state.current) return itx.reply({ embeds: [infoEmbed("ℹ️  ไม่มีเพลงกำลังเล่น", "")], flags: [64] });
      ...
      state.player.stop(true);       
      const nextLabel = state.queue.length ? `ถัดไป: **${state.queue[0]?.title || "—"}**` : (state.autoPlay ? "📻 สุ่มเพลงถัดไปจากสถานี..." : "คิวหมดแล้ว");
      return itx.reply({ embeds: [successEmbed("⏭️  ข้ามเพลงแล้ว", nextLabel)] });
    }
...
client.on("interactionCreate", async (itx) => {
  ...
  const needsSameVC = !["help","ping", "botupdate", "np", "queue", "autoplay"].includes(itx.commandName);
  ...
  if (itx.commandName === "autoplay") {
    state.autoPlay = !state.autoPlay;
    return itx.reply({ embeds: [successEmbed("📻  Auto-Play", `สถานะ: **${state.autoPlay ? "เปิดการใช้งาน" : "ปิดการใช้งาน"}**\n*(สุ่มเพลงจาก Cache เล่นอัตโนมัติเมื่อคิวหมด)*`)]});
  }
  ...
  if (itx.commandName === "stop") {
    ...
    const vc = getVoiceConnection(itx.guild.id);
    if (vc) vc.destroy();
    return itx.reply({ embeds: [successEmbed("🛑  หยุดเพลงแล้ว", "ล้างคิวและออกจากห้องเสียงเรียบร้อย")] });
  }

  if (itx.commandName === "pause") {
    state.player.pause();
    updateControlMsgIfNeeded(state);
    return itx.reply({ embeds: [infoEmbed("⏸️  หยุดชั่วคราว", "พิมพ์ `/resume` เพื่อเล่นต่อ")] });
  }

  if (itx.commandName === "resume") {
    state.player.unpause();
    updateControlMsgIfNeeded(state);
    return itx.reply({ embeds: [successEmbed("▶️  เล่นต่อแล้ว", `กำลังเล่น: **${state.current?.title || "—"}**`)] });
  }
  ...
  if (itx.commandName === "remove") {
    ...
    if (index < 1 || index > state.queue.length) {
      return itx.reply({ embeds: [errorEmbed(`หมายเลขลำดับไม่ถูกต้อง (มีอยู่ ${state.queue.length} เพลง)`)], flags: [64] });
    }
