...
// 🟢 อัปเกรดความฉลาดให้ getTitle ให้มันเช็ค Cache ก่อน จะได้ไม่ต้องรอ! 🟢
async function getTitle(input){
  try {
    // ⚡ FAST CACHE MATCH สำหรับชื่อเพลง
    if (config.cacheEnabled) {
      const ytMatch = input.match(/(?:youtu\.be\/|youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=))([^"&?\/\s]{11})/i);
      if (ytMatch && cacheDB[ytMatch[1]]) return cacheDB[ytMatch[1]].title; // ถ้าใส่ลิ้งค์มา เจอเลย ตอบเลย
      
      if (!isUrl(input)) {
        const lowerQ = input.toLowerCase();
        for (const data of Object.values(cacheDB)) {
          if (data.title.toLowerCase().includes(lowerQ)) return data.title; // ถ้าพิมพ์ชื่อมา เจอเลย ตอบเลย
        }
      }
    }

    // 📥 ถ้าไม่เจอใน Cache ค่อยไปถาม YouTube
    if (isSpotifyUrl(input)) { const t = await spotifyTitle(input); if (t) return t; }
    
    // ใช้ --flat-playlist เพื่อดึงเฉพาะชื่อ ไม่โหลดข้อมูลลึก ทำให้ไวขึ้น
...
client.on("messageCreate", async (msg) => {
  try {
    ...
    const cmd = ({ p: "play", q: "queue", now: "np", next: "skip", s: "skip", st: "stop", vol: "volume", upd: "botupdate", h: "help", help: "help" })[cmdRaw] || cmdRaw;
...
function createGuildState(guild) {
  ...
  const state = { queue: [], current: null, player, currentPipe: null, restartGuard: { tried: false }, currentResource: null, volumePct: config.defaultVolume, loopMode: config.defaultLoop, skipRequested: false, skipGeneration: 0, isHandlingTransition: false, isForceTransition: false, controlMsg: null };
...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  if (!state.queue.length) {
    ...
    const vc = getVoiceConnection(guild.id); if (vc) vc.destroy();
  ...
  try {
    ...
    logPretty("NOWPLAY", `${next.title}`, { user: next.requestedBy, tail: `up_next: ${state.queue[0]?.title || "—"}` });
...
// 🟢 💽 ระบบสถาปัตยกรรม CACHE แบบเต็มสูบ + FAST MATCHER 🟢
async function startPlayback(guild, item, state) {
  ...
  // ⚡ 1. FAST CACHE CHECK (อ่านลิ้งค์ตรง หรือ ค้นหาจากชื่อ)
  if (config.cacheEnabled) {
