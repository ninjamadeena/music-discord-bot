...
// ===============================================
// ⚡️ การแก้ไข: ย้ายการโหลดโมดูลพื้นฐานมาไว้ที่นี่
// ===============================================
const fs = require("fs");
...
const config = {
  ...
  logDir: process.env.LOG_DIR || path.join(process.cwd(), "logs"), // <-- ตอนนี้ 'path' ถูกกำหนดค่าแล้ว
  ...
  // Select the update channel for yt‑dlp: 'stable' (default) or 'nightly'.
  // The nightly channel includes the latest fixes and features, which may be
  // necessary to avoid HTTP 403 errors or other issues with YouTube.  If set
  // to 'nightly', runYtDlpUpdate will execute `yt-dlp --update-to nightly`;
  // otherwise it will run the standard `yt-dlp -U` to update to the latest
  // stable release.  You can override this via the YTDLP_UPDATE_CHANNEL
  // environment variable.
  ytdlpUpdateChannel: (process.env.YTDLP_UPDATE_CHANNEL || 'stable').toLowerCase(),

  // ============================================
  // Audio format selection for yt‑dlp.  The default "bestaudio/best"
  // sometimes picks an HLS stream which is protected by additional
  // cookies and results in 403 errors when ffmpeg retrieves the
  // segments.  To avoid this, you can force yt‑dlp to select a
  // direct file format (e.g. m4a).  Specify a format string here or
  // via the YTDLP_AUDIO_FORMAT environment variable.  See the yt‑dlp
  // documentation for format selection syntax.  The default below
  // prioritizes an m4a audio‑only stream and falls back to the
  // best audio or the best overall stream if no audio‑only format is
  // available.
  ytdlpAudioFormat: process.env.YTDLP_AUDIO_FORMAT || "bestaudio[ext=m4a]/bestaudio/best",
...
console.log("--------------------------------")
// บรรทัดเหล่านี้ถูกย้ายขึ้นไปด้านบน
/*
const fs = require("fs");
const path = require("path");
const http = require("http");
...
function ytdlpOpts(extra = {}) {
  ...
  //
  // yt-dlp requires a JavaScript runtime for some YouTube player clients.  When
  // none is available, yt‑dlp will warn and may miss formats.  To silence
  // these warnings and choose a set of clients that work without an external
  // runtime, specify extractorArgs.  The default clients (tv, android_sdkless,
  // web) require decryption via an embedded JS interpreter.  By setting
  // player_client=default we let yt-dlp pick a safer fallback order that does
  // not depend on an external JS runtime【889944230260670†L2010-L2044】.  This has
  // been recommended by the yt-dlp maintainers as a workaround when a JS
  // runtime is not installed.  Users can override this behaviour by passing
  // their own extractorArgs via the `extra` argument.
  const defaultExtractorArgs = { youtube: { player_client: 'default' } };
  // Merge any caller‑supplied extractorArgs with the default.  The caller
  // overrides take precedence.  Note: this is a shallow merge; deep merging
  // beyond the first level is unnecessary for our use case.
  const mergedExtractorArgs = {
    ...(defaultExtractorArgs),
    ...(extra.extractorArgs || {}),
  };
  const opts = { ...base, extractorArgs: mergedExtractorArgs, ...extra };
  return opts;
}
...
// Wrap a string with an ANSI colour code for console output
...
// Append a log line to the bot log files; debug lines go to the debug log only.
...
// Print and log a formatted message with colouring; debug messages respect the DEBUG_FFMPEG flag.
...
async function runYtDlpUpdate(replyFn){
  ...
  try {
    // Probe current version, ignore any error
    try { await ytdlp("--version"); } catch {}
    // Decide which update command to run based on configured channel
    let out;
    if (config.ytdlpUpdateChannel === 'nightly') {
      out = await ytdlp("--update-to", "nightly").catch(err => ({ error: err }));
    } else {
      out = await ytdlp("-U").catch(err => ({ error: err }));
    }
    if (out?.error) {
...
async function getDirectAudioUrlAndHeaders(input) {
  // Always request a single JSON dump and use the configured audio
  // format selector.  Using the configured format avoids HLS streams
  // that can cause HTTP 403 errors during ffmpeg playback.  See
  // config.ytdlpAudioFormat for the default string and the README for
  // environment variable details.
  const format = config.ytdlpAudioFormat;
  const info = await ytdlp(input, ytdlpOpts({ dumpSingleJson: true, f: format }));
  const url = info?.url;
...
async function startPlayback(guild, item, state) {
  ...
  const pageUrl = await resolveFirstVideoUrl(tem.source);
  if (!pageUrl) {
