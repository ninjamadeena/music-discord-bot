...
async function startPlayback(guild, item, state) {
  ...
  if (config.cacheEnabled) {
    // ดักจับลิ้งค์ YouTube เพื่อดึง Video ID ตรงๆ เช่น 5legRVBfFxk
    ...
    if (ytMatch) {
      ...
      // ดักจับข้อความค้นหา (เช็คกับชื่อ Title ที่เคยโหลดไว้แล้ว)
  ...
  if (fastCacheHit) {
    // 🎯 เจอใน CACHE ตั้งแต่แรก -> เล่นทันที 0ms!
    ...
    // 📥 2. CACHE MISS -> ต้องไปขอข้อมูลจาก YouTube ก่อน
    if (isSpotifyUrl(source)) {
      ...
      source = `ytsearch1:${source}`; // แปลงคำค้นให้ yt-dlp รู้จัก
    }
    ...
    // 🛠️ FIX BUG: ถ้าคืนค่ามาเป็น Search List (Playlist) ให้หยิบอันแรกสุดมาใช้
    ...
    videoId = info.id; // ตอนนี้จะได้ 5legRVBfFxk เสมอ!
    title = info.title || "Unknown Title";
    const duration = info.duration_string || `${info.duration}s`;
    targetUrl = `https://www.youtube.com/watch?v=${videoId}`; // สร้างลิ้งค์ตรงที่ถูกต้อง
    filePath = path.join(CACHE_DIR, `${videoId}.ogg`);

    // ⚡ Secondary Cache Check (เผื่อผู้ใช้พิมพ์คำค้นหาใหม่ แต่ดันได้เพลงเดิมที่มีอยู่แล้ว)
    if (config.cacheEnabled && cacheDB[videoId] && fs.existsSync(cacheDB[videoId].file_path)) {
       ...
       // โหลดโดยใช้ targetUrl ลิ้งค์ตรง (ปลอดภัย 100%)
       const dlArgs = [
