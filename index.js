...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  try {
    ensureVC(guild, next.voiceChannelId, state);

    const pageUrl = await resolveFirstVideoUrl(next.source);
    if (!pageUrl) {
      logPretty("ERROR", "cannot resolve page url, skip", { tail: `q="${next.source}"` });
      await sendToTextChannel(guild, next.textChannelId, `⚠️ เล่นไม่ได้ ข้าม: **${next.title}**`);
      state.current = null;
      return playNext(guild, textChannelId, state);
    }
