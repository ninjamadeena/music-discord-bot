...
client.on("messageCreate", async (msg) => {
  try {
    ...
    if (cmd === "skip") {
      ...
      cleanupCurrentPipeline(state); // 👈 ฆ่าเพลงเก่าก่อน!
      state.player.stop(true);       // 👈 แล้วค่อยสั่งหยุดเพื่อดึงเพลงถัดไป
    ...
    if (cmd === "stop") {
      ...
      cleanupCurrentPipeline(state); // 👈 ฆ่าทิ้งก่อน
      state.player.stop(true);       // 👈 เคลียร์สถานะ
...
async function handlePlayerError(error, guild, state) {
  ...
  // Guard: ถ้าผู้ใช้สั่งข้าม/หยุด เอง ให้เมิน Error ที่พ่นออกมา
...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  if (!state.queue.length) {
    state.current = null;
    state.isForceTransition = false; // 👈 เคลียร์สถานะข้ามเพลงเมื่อคิวว่างแล้ว
  ...
  try {
    ...
    // รอให้เสียงเล่นจริงๆ ก่อนส่ง Now Playing
    await new Promise((resolve) => {
      // ถ้าสถานะเป็น Playing แล้ว ออกทันที
      ...
      // Fallback เผื่อโหลดนานเกินไป ให้หยุดรอและข้ามไปแสดง Now Playing ได้เลย (15 วินาที)
    ...
    state.isForceTransition = false; // 👈 เคลียร์สถานะถ้าดึงเพลงล้มเหลว
...
client.on("interactionCreate", async (itx) => {
  if (itx.isButton()) {
    ...
    if (id === "mc_skip") {
      ...
      cleanupCurrentPipeline(state); // 👈 ฆ่าเก่าทิ้ง
      state.player.stop(true);       // 👈 ดึงใหม่
    ...
    if (id === "mc_stop") {
      ...
      cleanupCurrentPipeline(state); // 👈 ฆ่าทิ้ง
      state.player.stop(true);       // 👈 หยุด
...
client.on("interactionCreate", async (itx) => {
  ...
  if (itx.commandName === "skip") {
    ...
    cleanupCurrentPipeline(state); // 👈 ฆ่าก่อน
    state.player.stop(true);       // 👈 หยุด
  ...
  if (itx.commandName === "stop") {
    ...
    cleanupCurrentPipeline(state); // 👈 ฆ่าทิ้ง
    state.player.stop(true);       // 👈 หยุดเล่น
    const vc = getVoiceConnection(itx.guild.id);
    if (vc) vc.destroy();
