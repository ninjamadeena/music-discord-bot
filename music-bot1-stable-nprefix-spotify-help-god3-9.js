...
function createGuildState(guild) {
  ...
  player.on(AudioPlayerStatus.Idle, () => {
    ...
    handlePlayerIdle(guild, state)
      .catch((e) => logPretty("ERROR", `Idle handler error: ${e?.message || e}`))
      .finally(() => { state.isHandlingTransition = false; });
  });
  player.on("error", (e) => {
...
async function handlePlayerIdle(guild, state) {
  ...
  if (state.loopMode === "track" && !manualSkip) {
    ...
    try {
      ...
      logPretty("ERROR", `loop track restart error: ${e?.message || e}`);
      state.current = null;
...
async function handlePlayerError(error, guild, state) {
  ...
  // ดึงข้อมูลที่จำเป็นออกมาก่อน ก่อนที่ async ใดๆ จะ clear state.current
  if (!state.current) return;
  if (state.isHandlingTransition) return; // ป้องกัน double-handle

  // ตั้ง flag ก่อนเลย ป้องกัน Idle handler แทรกได้
  state.isHandlingTransition = true;
  ...
  if (!state.restartGuard.tried) {
    ...
    try {
      ...
      logPretty("ERROR", `retry failed: ${e?.message || e}`);
      state.current = null;
      await playNext(guild, savedChannelId, state);
    }
    state.isHandlingTransition = false;
  ...
  // guard.tried เป็น true แล้ว → ข้ามเพลงนี้เลย
  state.current = null;
  await playNext(guild, savedChannelId, state);
  state.isHandlingTransition = false;
}
...
async function playSame(guild, textChannelId, item, state = getGuildState(guild)) {
  // NOTE: ไม่ reset restartGuard.tried ที่นี่ เพราะถ้า playSame fail อีก
  // handlePlayerError จะเห็นว่า tried=true แล้วและ skip ไปเลย ป้องกัน infinite loop
  state.current = item;
  cleanupCurrentPipeline(state);
  // โยน error ขึ้นไปให้ caller จัดการ (handlePlayerError)
