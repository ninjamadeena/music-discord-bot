...
// Log files:
//   bot.log        — activity ทั่วไป (COMMAND, MUSIC, INFO, SYSTEM)
//   bot-error.log  — เฉพาะ ERROR และ WARN เท่านั้น → เปิดดูตอนมีปัญหา
//   bot-debug.log  — ทุกอย่างรวม ffmpeg output → ใช้ตอน debug ลึกๆ
...
function writeLog(plain, isDebug = false, type = "") {
  // bot-debug.log — รับทุกอย่างเสมอ (รวม ffmpeg)
  try { fs.appendFileSync(LOG_FILE_DEBUG, plain + "\n", "utf8"); } catch {}

  // bot-error.log — เฉพาะ ERROR และ WARN เท่านั้น
  if (type === "ERROR" || type === "WARN") {
    try { fs.appendFileSync(LOG_FILE_ERROR, plain + "\n", "utf8"); } catch {}
  }

  // bot.log — ทุกอย่างยกเว้น ffmpeg debug
  if (!isDebug) {
...
client.on("messageCreate", async (msg) => {
  try {
    ...
    if (cmd === "skip") {
      ...
      // Disable ปุ่มของ Now Playing เดิมก่อน skip
      if (state.controlMsg) {
        try {
...
function createGuildState(guild) {
  ...
  const state = {
    ...
    skipRequested: false,
    skipGeneration: 0, // เพิ่ม: generation counter เพื่อให้ error handler รู้ว่า error นี้เกิดจาก skip ของ generation ไหน
    isHandlingTransition: false,
    controlMsg: null, // reference to Now Playing message (for button updates)
  };

  player.on(AudioPlayerStatus.Idle, () => {
    // ถ้า error handler กำลัง retry อยู่ ไม่ต้องทำ Idle เพราะจะ race กัน
...
function spawnUniversalPipe(source) {
  ...
  if (isTikTok) {
    // TikTok: ต้องการ impersonation เพื่อหลีกเลี่ยง 403
    // และต้องเลือก format ที่มี audio เท่านั้น (บางคลิปมีแค่ video stream)
    ytArgs.push(
      "--impersonate", "chrome",
  ...
  if (config.ffmpegLowLatency) {
    a.push("-fflags", "+nobuffer", "-flags", "low_delay",
      "-analyzeduration", String(config.ffmpegInputAnalyzeMs * 1000), "-probesize", "32k");
  } else {
  ...
  ff.stdout.on("error", swallowPipeError);
  ff.stderr.on("error", swallowPipeError);
  // *** EPIPE guard: เมื่อ stop/skip kill ffmpeg, yt-dlp จะ write เข้า stdin ที่ปิดแล้ว
  // ถ้าไม่มี handler ตรงนี้ Node จะ throw unhandled EPIPE แล้วตาย ***
...
async function handlePlayerIdle(guild, state) {
  ...
  if (!finished) return; // ป้องกัน race: ถ้า error handler clear ไปแล้ว
  ...
  // bump playGeneration เพื่อให้ error handler รู้ว่า error ที่มาหลังจากนี้
  // เป็น stale error จากเพลงเก่า ไม่ใช่เพลงใหม่
...
async function handlePlayerError(error, guild, state) {
  ...
  if (!state.current) return;
  if (state.isHandlingTransition) return; // ป้องกัน double-handle

  const currentItem = state.current;
  const savedChannelId = currentItem.textChannelId;

  // ── Guard 1: skip-induced errors ──────────────────────────────────────────
  // skipGeneration bump ทุกครั้งที่ user กด skip
  // error ที่มาจาก generation เดียวกับที่ skip ล่าสุด = stale error จากการ kill ffmpeg
  if (state.skipRequested || state._lastErrorGen === state.skipGeneration) {
    logPretty("LOG", `Player error after skip (ignored): ${error?.message || error}`);
    return;
  }

  // ── Guard 2: normal song-transition errors ────────────────────────────────
  // playGeneration bump ทุกครั้งที่ Idle handler จบเพลงแล้วเริ่มเพลงถัดไป
  // error ที่มาจาก generation ก่อนหน้า = ffmpeg เก่าตายทีหลัง ไม่เกี่ยวกับเพลงใหม่
  // เก็บ _activePlayGen ตอน startPlayback เพื่อเทียบ
  if (state._lastIdleGen !== undefined && state._activePlayGen !== state._lastIdleGen + 1) {
    // นับง่ายๆ: ถ้า playGeneration ตอนนี้ต่างจากตอนที่เราเริ่มเล่น = เพลงนี้เป็นเพลงใหม่แล้ว
  }
  // วิธีที่ถูกต้อง: เช็คว่า error generation ตรงกับเพลงที่กำลังเล่นอยู่จริงๆ
  ...
  // บันทึก skip generation ไว้กัน error ซ้ำ
...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  if (!state.queue.length) {
    ...
    // Disable control buttons on the last Now Playing message
    if (state.controlMsg) {
      try {
        await state.controlMsg.edit({
          components: [new ActionRowBuilder().addComponents(
  ...
  try {
    ...
    // รอให้ ffmpeg ส่ง audio data จริงๆ ออกมาก่อน (ไม่ใช่แค่ probe)
    // เพราะ AudioPlayerStatus.Playing fire ตั้งแต่ demuxProbe เสร็จ ยังไม่มีเสียงจริง
    await new Promise((resolve) => {
      const pipe = state.currentPipe;
      if (!pipe) return resolve();
      let resolved = false;
      const done = () => { if (!resolved) { resolved = true; resolve(); } };

      // รอ data packet แรกจาก ffmpeg (= เสียงจริงเริ่มออกมาแล้ว)
      pipe.stream.once("data", done);
      // fallback: ถ้า stream ตายก่อน ให้ resolve ด้วย
      pipe.stream.once("end",   done);
      pipe.stream.once("error", done);
      // fallback timeout 20 วิ กันค้าง
      const t = setTimeout(done, 20000);
      // cleanup listener ถ้า resolve แล้ว
      pipe.stream.once("data", () => clearTimeout(t));
    });

    // ตรวจว่า skip/stop ไปแล้วหรือเปล่าก่อนส่ง Now Playing
...
async function startPlayback(guild, item, state) {
  ...
  // บันทึก playGeneration ตอนนี้ไว้ เพื่อให้ error handler รู้ว่า error นี้เป็นของเพลงนี้จริงๆ
  // ถ้า playGeneration เปลี่ยน (เพลงจบ/skip) error ที่มาทีหลังจะถูก ignore
  ...
  // All sources (YouTube, TikTok, SoundCloud, …) go through the yt-dlp→ffmpeg
  // pipe. This avoids CDN URL expiry (YouTube) and 403 errors (TikTok) because
  // yt-dlp owns the download instead of ffmpeg fetching over HTTP.
