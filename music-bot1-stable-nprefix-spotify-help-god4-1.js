...
// 🟢 ฟังก์ชันนี้คือตัวสำคัญที่หายไป เอาไว้ยิงสตรีมสดกรณีโหลดไฟล์ลงฮาร์ดดิสก์ไม่สำเร็จ
function spawnUniversalPipe(source) {
  ...
  // เพิ่มการใช้ JS Runtime ให้กับระบบเล่นสดเพื่อแก้ปัญหา n challenge
...
async function startPlayback(guild, item, state) {
  ...
  const info = await ytdlp(ytdlpArgsInfo).catch(() => null);
  ...
  // 🟢 ยิง URL ตรงไปที่ Video ID เพื่อป้องกันการค้นหาซ้ำแบบผิดพลาด
  ...
  if (config.cacheEnabled) {
    if (cacheDB[videoId] && fs.existsSync(cacheDB[videoId].file_path)) {
      ...
      // 🟢 เพิ่ม --js-runtimes node ในขั้นตอนโหลด และใช้ targetUrl ที่เป็น Youtube Link โดยตรง
      const dlArgs = [
  ...
  if (playFromCache) {
      ...
      const pipeObj = spawnUniversalPipe(targetUrl); // ใช้ targetUrl แทน source เพื่อแก้ปัญหาการค้นหา
      state.currentPipe = pipeObj;
