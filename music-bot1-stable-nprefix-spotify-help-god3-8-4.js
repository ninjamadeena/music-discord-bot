...
async function handlePlayerIdle(guild, state) {
  // snapshot pipe ก่อน cleanup เพื่อทำเครื่องหมาย "หมดอายุ"
  const idlePipe = state.currentPipe;
  cleanupCurrentPipeline(state);
  ...
  // ทำเครื่องหมาย pipe เก่าว่า "done" เพื่อกรอง stale errors ที่มาทีหลัง
  if (idlePipe) idlePipe._done = true;
...
async function handlePlayerError(error, guild, state) {
  ...
  // ── Guard 2: stale errors จากเพลงก่อนหน้า (normal song transition) ────────
  // activePipe คือ pipe ที่เรา snapshot ไว้ตอน startPlayback
  // ถ้า pipe นั้นถูก Idle handler mark _done = true แล้ว = error มาจากเพลงเก่า
  if (state.activePipe && state.activePipe._done) {
    logPretty("LOG", `Player error from previous song (ignored, stale pipe): ${error?.message || error}`);
    return;
  }
...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  try {
    ...
    // รอให้ player เข้าสถานะ Playing จริงๆ ก่อนส่ง Now Playing embed
    // AudioPlayerStatus.Playing fire เมื่อ discord.js ได้รับ opus packet แรกจาก discord gateway
    // = เสียงถึง Discord แล้วจริงๆ (ไม่ใช่แค่ ffmpeg start)
    await new Promise((resolve) => {
      if (!state.current || state.current !== next) return resolve();
      let resolved = false;
      const done = () => { if (!resolved) { resolved = true; resolve(); } };

      state.player.once(AudioPlayerStatus.Playing, done);
      // fallback: ถ้า stream จบเร็วหรือ error ก็ resolve เลยไม่ค้าง
      state.player.once(AudioPlayerStatus.Idle, done);
      state.player.once("error", done);
      // timeout 25 วิ กันค้างถ้า yt-dlp ช้ามาก
      const t = setTimeout(done, 25000);
      state.player.once(AudioPlayerStatus.Playing, () => clearTimeout(t));
    });
...
async function startPlayback(guild, item, state) {
  ...
  // snapshot pipe ไว้ใน activePipe เพื่อให้ error handler ตรวจสอบ
  // ถ้า activePipe._done = true แปลว่า Idle handler จัดการจบแล้ว error นี้เป็น stale
  state.activePipe = pipeObj;
