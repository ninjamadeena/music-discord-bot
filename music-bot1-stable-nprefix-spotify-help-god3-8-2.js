...
async function handlePlayerError(error, guild, state) {
  ...
  // ถ้าเป็น error จากการ skip (Premature close / Connection reset) ไม่ต้อง retry
  // เพราะ Idle handler จะจัดการต่อเองอยู่แล้ว
  // ใช้ skipGeneration แทน skipRequested เพราะ skipRequested อาจถูก reset ไปแล้ว
  // ตอน Idle handler ทำงานก่อน error handler (race condition)
  ...
  // บันทึก generation ปัจจุบันไว้ เพื่อกัน error ซ้ำจาก generation เดียวกัน
...
async function playNext(guild, textChannelId, state = getGuildState(guild)) {
  ...
  try {
    ...
    // รอให้ player เข้าสถานะ Playing จริงก่อนค่อยส่ง Now Playing embed
    // เพื่อไม่ให้ embed ขึ้นก่อนเสียงจริงๆ ออกมา (yt-dlp + ffmpeg ยังโหลดอยู่)
    await new Promise((resolve) => {
      const onPlaying = () => { resolve(); };
      const onIdle = () => { resolve(); }; // fallback ถ้า stream สั้นมาก
    ...
    // ตรวจว่า skip ไปแล้วหรือเปล่าก่อนส่ง Now Playing
