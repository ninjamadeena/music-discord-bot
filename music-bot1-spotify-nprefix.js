...
const PREFIX = (process.env.PREFIX || "n!").trim();
...
function buildPseudoItxFromMessage(msg, commandName, opts){
  let repliedMsg = null;
  const pseudo = {
    commandName,
    user: msg.author,
    ...
    reply: async (payload) => {
      // Discord message replies don't support "ephemeral"; strip it if present.
      if (typeof payload === "string") {
        repliedMsg = await msg.reply({ content: payload });
        return repliedMsg;
      }
      const p = { ...(payload || {}) };
      delete p.ephemeral;
    ...
    deferReply: async (_payload) => {
      // Send a lightweight placeholder we can edit later
      repliedMsg = await msg.reply({ content: "⏳ กำลังทำงาน..." });
      return repliedMsg;
    },
...
client.on("messageCreate", async (msg) => {
  try {
    ...
    const pseudo = buildPseudoItxFromMessage(msg, commandName, opts);
