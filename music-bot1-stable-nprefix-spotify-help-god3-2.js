...
// Resolve the actual yt-dlp binary path from the yt-dlp-exec package so we
// can spawn it directly as a child process (spawn("yt-dlp") fails on many
// hosts because the binary is NOT on PATH — it lives inside node_modules).
const YTDLP_BIN = (() => {
  try {
    const m = require("yt-dlp-exec");
    // yt-dlp-exec ≥ 1.x: the callable itself is the wrapper; the binary is at m.raw or m.path
    if (m && typeof m.raw === "string") return m.raw;
    if (m && typeof m.path === "string") return m.path;
  } catch {}
  // Try resolving common inner paths used by different versions of the package
...
// Universal yt-dlp -> ffmpeg pipe. Works for YouTube, TikTok, SoundCloud and
// any other platform yt-dlp supports. yt-dlp downloads the best audio and
// writes raw bytes to stdout; ffmpeg reads from stdin, re-encodes to Opus and
// writes to its own stdout for Discord.
function spawnYtdlpPipe(pageUrl) {
  if (!FFMPEG_AVAILABLE) throw new Error("ffmpeg binary not available");
  ...
  ytArgs.push(
    ...
    "--fragment-retries", "infinite",
    "--js-runtimes", "node",
    // extractor args help bypass YouTube's bot detection
  ...
  helper.on("error", (e) => logPretty("ERROR", "yt-dlp(pipe) spawn error: " + (e?.message || e)));
  helper.stderr.on("error", swallowPipeError);
  helper.stderr.on("data", (d) => {
    try { logPretty("LOG", "[yt-dlp] " + d.toString().trim()); } catch {}
  });
  ...
  ff.on("error",    (e) => logPretty("ERROR", "ffmpeg(pipe) spawn error: " + (e?.message || e)));
  ff.stdout.on("error", swallowPipeError);
  ff.stderr.on("error", swallowPipeError);
  ff.stderr.on("data",  (d) => {
    try { logPretty("LOG", "[ffmpeg] " + d.toString().trim()); } catch {}
  });
...
function spawnTikTokPipe(pageUrl) {
  ...
  const helper = spawn(YTDLP_BIN, ytdlpArgs, {
...
async function startPlayback(guild, item, state) {
  ...
  // Always use yt-dlp -> ffmpeg pipe mode.
  // Previously we resolved a direct audio URL and had ffmpeg fetch it over HTTP,
  // but YouTube now expires those CDN URLs extremely quickly (sometimes within
  // milliseconds), causing ffmpeg to receive an empty or truncated stream and
  // the player to fire FINISHED instantly. By letting yt-dlp own the download
  // and piping its stdout directly into ffmpeg we avoid the URL-expiry window
  // entirely. TikTok and every other platform benefit from the same approach.
  const pipeObj = spawnYtdlpPipe(pageUrl);
